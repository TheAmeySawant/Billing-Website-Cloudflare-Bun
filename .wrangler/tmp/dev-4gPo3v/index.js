...
// .wrangler/tmp/bundle-m8bMJn/middleware-loader.entry.ts
init_modules_watch_stub();

// .wrangler/tmp/bundle-m8bMJn/middleware-insertion-facade.js
init_modules_watch_stub();
...
// src/server/middleware/auth.ts
var accessAuth = createMiddleware(async (c2, next) => {
  if (c2.env.ENVIRONMENT === "development") {
    await next();
  }
  if (!c2.env.POLICY_AUD) {
    return c2.json("Missing required audience", 403);
  }
  const token = c2.req.header("cf-access-jwt-assertion");
  if (!token) {
    return c2.json("Missing required CF Access JWT", 403);
  }
  try {
    const JWKS = createRemoteJWKSet(
      new URL(`${c2.env.CF_ACCESS_DOMAIN}/cdn-cgi/access/certs`)
    );
    await jwtVerify(token, JWKS, {
      issuer: c2.env.CF_ACCESS_DOMAIN,
      audience: c2.env.POLICY_AUD
    });
    await next();
  } catch (error) {
    const message2 = error instanceof Error ? error.message : "Unknown error";
    return c2.json(`Invalid token: ${message2}`, 403);
  }
});
...
var buildDate = /* @__PURE__ */ __name((year3, month, day2, time2) => {
...
var validateDayOfMonth = /* @__PURE__ */ __name((year3, month, day2) => {
...
var isLeapYear = /* @__PURE__ */ __name((year3) => {
...
// src/server/index.ts
var app = new Hono2();
app.use(accessAuth).get("/api/health", (c2) => c2.json("Healthy! \u{1F525}"));
app.use(accessAuth).get("/api/dashboard", (c2) => c2.json("Soham Dharap's Dashboard"));
app.use(accessAuth).get("/api/clients", async (c2) => {
  try {
    const result = await c2.env.billingDB.prepare("SELECT * FROM clients ORDER BY id DESC").all();
    return c2.json(
      {
        success: true,
...
app.use(accessAuth).post("/api/new/client", async (c2) => {
  try {
    ...
    const result = await c2.env.billingDB.prepare(
...
app.use(accessAuth).post("/api/new/qrcode", async (c2) => {
  try {
    const { image } = await c2.req.json();
    if (!image || !image.startsWith("data:image")) {
      return c2.json({ success: false, error: "Invalid image data" }, 400);
    }
    const matches = image.match(/^data:([A-Za-z-+\/]+);base64,(.+)$/);
    if (!matches || matches.length !== 3) {
      return c2.json({ success: false, error: "Invalid base64 string" }, 400);
    }
    const contentType = matches[1];
    const buffer = Buffer.from(matches[2], "base64");
    const s3 = new S3Client({
      region: "us-east-1",
      endpoint: c2.env.B2_ENDPOINT,
      credentials: {
        accessKeyId: c2.env.B2_ACCESS_KEY_ID,
        secretAccessKey: c2.env.B2_SECRET_ACCESS_KEY
      }
    });
    await s3.send(
      new PutObjectCommand({
        Bucket: c2.env.B2_BUCKET_NAME,
        Key: "QRcode/current-qr.img",
        // Generic extension or fixed
        Body: buffer,
        ContentType: contentType
      })
    );
    return c2.json({ success: true, message: "QR Code uploaded successfully" });
  } catch (error) {
...
app.use(accessAuth).get("/api/qrcode", async (c2) => {
  try {
    const s3 = new S3Client({
      region: "us-east-1",
      endpoint: c2.env.B2_ENDPOINT,
    ...
    if (!bodyByteArray) {
      return c2.json({ success: false, error: "Empty file" }, 404);
    }
    c2.header("Content-Type", response.ContentType || "image/png");
    return c2.body(bodyByteArray.buffer);
  } catch (error) {
    console.error("Error fetching QR code:", error);
    return c2.json({ success: false, error: "QR Code not found" }, 404);
  }
});
