...
var accessAuth = createMiddleware(async (c, next) => {
  ...
  try {
    ...
    return c.json(`Invalid token: ${message2}`, 403);
  }
});

// src/server/index.ts
var app = new Hono2();
app.use(accessAuth).get("/api/health", (c) => c.json("Healthy! \u{1F525}"));
app.use(accessAuth).get("/api/dashboard", (c) => c.json("Soham Dharap's Dashboard"));
app.use(accessAuth).get("/api/clients", async (c) => {
  try {
    const result = await c.env.billingDB.prepare("SELECT * FROM clients ORDER BY id DESC").all();
    return c.json(
      {
        success: true,
...
app.use(accessAuth).post("/api/new/client", async (c) => {
  try {
    ...
    const result = await c.env.billingDB.prepare(
...
// .wrangler/tmp/bundle-OHFaIl/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-OHFaIl/middleware-loader.entry.ts
